...
* **Target Variable ($\mathbf{Default}$):** Created from `loan_status` where ('default' and '90+dpd') are classified as **1 (Default)**.
...
* **Income Distribution by Default Status:** ![Income Distribution by Default Status](Credit-Risk-PD-Modeling/images/vis-1.png)
...
* **Correlation Heatmap:** ![Correlation Heatmap](Credit-Risk-PD-Modeling/images/vis-2.png)
...
* **Default Rate by Loan Purpose:** ![Default Rate by Loan Purpose](Credit-Risk-PD-Modeling/images/vis-3.png)
...
| :--- | :--- | :--- |
| **Confusion Matrix** | $\begin{bmatrix} 396 & 0 \\ 104 & 0 \end{bmatrix}$ | **Critical Failure: Zero True Positives ($\text{TP}=0$)**. The model only predicted the majority class (Non-Default). |
| **AUC** | $0.4896$ | Worse than random guessing. |

### Coefficient Interpretation (Baseline)

| Feature | Odds Ratio ($\mathbf{e^\beta}$) | Finding |
| :--- | :--- | :--- |
| $\mathbf{DTI\_Ratio}$ | $\mathbf{1.871}$ | **Strongest Predictor:** A one-unit increase in DTI increases the odds of default by **87.1\%**. (Intuitive) |
| $\mathbf{credit\_score}$ | $1.128$ | **Counter-Intuitive:** An increase in credit score increases the odds of default, signaling model instability due to imbalance. |
